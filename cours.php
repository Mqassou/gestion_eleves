...
<?php
			...
			echo("<div  class='col-md-4  col-md-offset-1'><td><a href='majCours.php?id=" . $result->id_cours . "' alt='MàjCours' title='Mise à jour'>Mettre à jour ce cours</a></td></tr></div></br>");
			 ...
			 if(isset($_POST["add_cours"]))
			 {
				...
				$DE = $_POST['coeffDe_cours'];
				if(!empty($_POST['coeffCe_cours'])){
				$CE = $_POST['coeffCe_cours'];
				}
				else{
					$CE = 0;
				}
				...
				try { 
				$requete="INSERT INTO `cours` (`nom_cours`,`coefficient` ,`coef_TP`,`coef_PRJ`,`coef_DE`,`coef_CE`,`id_prof`) VALUES ('$nom','$coef', '$TP','$PRJ','$DE','$CE','$num_prof')";
					$pdo->query($requete);
