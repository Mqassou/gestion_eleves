...
<?php echo("<p><h1> Mise à jour du groupe".$id."</h1></p>");?>
...
<?php
			...
			echo("<div class=' center-block col-md-12' ><p><h2> Les differents elèves du groupes sont:</p></h2><div>");
			$req_search = "SELECT * FROM eleve WHERE Num_groupe=$id";
			...
			<p><tr><td> Eleve: " . $result->nom_eleve . "</td></tr></p>
			</div></div>");
			echo("<div class='form-group'>
			<label >Nouveau groupe</label>
			<input type='text' name='num_groupe' class='form-control'  placeholder='Entrez un numero du groupe vers lequel déplacer cet elève'/></div>");
			echo("<div class='form-group'>
			...
			$requete_desinscription="DELETE FROM evalue WHERE matricule = '$nom'";
			try{
			$pdo->query($requete_desinscription);
			}
			...
			try{
				...
				header("refresh:0;url=majGroupe.php?id=".$id);
			}
